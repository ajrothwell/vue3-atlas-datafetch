...
export const useParcelsStore = defineStore('ParcelsStore', {
  ...
  actions: {
    async fillPwdParcelData() {
      ...
      const pwdParcelId = AddressStore.addressData.features[0].properties.pwd_parcel_id;
      // console.log('fillPwdParcelData, AddressStore.addressData.features[0].properties.pwd_parcel_id');
      const response = await fetch(`https://services.arcgis.com/fLeGjb7u4uXqeF9q/ArcGIS/rest/services/PWD_PARCELS/FeatureServer/0/query?where=PARCELID=%27${pwdParcelId}%27&outSR=4326&f=geojson&outFields=*&returnGeometry=true`);
